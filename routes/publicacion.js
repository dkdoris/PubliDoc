...
router.post('/crearPublicacionE', function(solicitud, respuesta, next) {
  ...
  var fechaP = new Date();       
  var crearPublicacionE=db.query('INSERT INTO Publicacion(fecha_Publicacion, fecha_Encuentro,ubicacion_Encuentro,lugar_Reside,detalles,foto,borrado_Logico,id_Usuario,tipo_Publicacion,numero_Documento) VALUES(?,?,?,?,?,?,?,?,?,?)', [fechaP,fecha,solicitud.body.ubicacion_Encuentro,solicitud.body.lugar_Reside,solicitud.body.detalles,solicitud.body.foto,0,solicitud.body.id_Usuario,solicitud.body.tipo_Publicacion,solicitud.body.numero_Documento],function(error,res,filas){
    if(error){
        console.log(error);
    }else{
...
router.post('/crearPublicacionP', function(solicitud, respuesta, next) {
    ...
    var fecha=year + "/" + month + "/" + day; 
    var fechaP = new Date();    
var crearPublicacionP=db.query('INSERT INTO Publicacion(fecha_Publicacion,fecha_Perdida,ubicacion_Perdida,detalles,borrado_Logico,id_Usuario,tipo_Publicacion,numero_Documento) VALUES(?,?,?,?,?,?,?,?)', [fechaP,fecha,solicitud.body.ubicacion_Perdida,solicitud.body.detalles,0,solicitud.body.id_Usuario,solicitud.body.tipo_Publicacion,solicitud.body.numero_Documento],function(error,res,filas){
    if(error){
      console.log(error);
   }else{
    //devuelve la respuesta json el servidor tabla intermedia
...
router.post('/listaPublicacion', function(solicitud, respuesta, next) { 
  ...
  var mostrarLista=db.query("SELECT Publicacion.id_Publicacion,tipo_Publicacion,fecha_Publicacion,numero_Documento from Publicacion where Publicacion.id_Usuario=? and Publicacion.borrado_Logico=?",[solicitud.body.idIdentificacion,0],function(error,resBD,filas){
    if(error){
      console.log(error);
    }else{
...
router.put('/modificarPublicacionP', function(solicitud, respuesta, next) {
  ...
  var year = todayTime.getFullYear();
  var fecha=year + "/" + month + "/" + day; 
  var modificarPublicacionE=db.query('UPDATE Publicacion SET fecha_Perdida=?,ubicacion_Perdida=?,detalles=?,numero_Documento=? WHERE id_Publicacion=?', [fecha,solicitud.body.ubicacion_Perdida,solicitud.body.detalles,solicitud.body.numero_Documento,solicitud.body.id_Publicacion],function(error,resBD,filas){
    if(error){
      console.log(error);
  }else{
      var modificarPublicacionDoc=db.query('DELETE FROM Publicacion_Documento WHERE id_Publicacion=?',[solicitud.body.id_Publicacion],function(error,res,filas){
