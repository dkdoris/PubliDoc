...
router.post('/listaAnuncios', function(solicitud, respuesta, next) { 
  var mostrarLista=db.query("SELECT id_Publicacion,tipo_Publicacion,fecha_Publicacion,numero_Documento from Publicacion where id_Usuario!=? and borrado_Logico=? and numero_Documento=?",[solicitud.body.idIdentificacion,0,solicitud.body.cedulatenporal],function(error,resBD,filas){
    if(error){
      console.log(error);
    }else{
